...
template sudoku() {
    ...
    //Here we initialize a  9x9 matrix
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            rp_puzzle[i][j] = RangeProof(4);
            rp_puzzle[i][j].range[0] <== 0; //GTE lower bound  
            rp_puzzle[i][j].range[1] <== 9; //LTE upper bound

            rp_puzzle[i][j].in <== puzzle[i][j];
            rp_puzzle[i][j].out === 1;

            rp_solution[i][j] = RangeProof(4);
            rp_solution[i][j].range[0] <== 0; //GTE lower bound 
            rp_solution[i][j].range[1] <== 9; //LTE upper bound

            rp_solution[i][j].in <== solution[i][j]; // assign and contrain proposed solution
            rp_solution[i][j].out === 1; //output must be 1 to be a valid solution
